...
from lib.stylegan2.torch_utils import misc
from lib.stylegan2.torch_utils import persistence
from lib.stylegan2.torch_utils.ops import conv2d_resample
from lib.stylegan2.torch_utils.ops import upfirdn2d
from lib.stylegan2.torch_utils.ops import bias_act
from lib.stylegan2.torch_utils.ops import fma
...
class Generator(torch.nn.Module):
    ...
    def style_mix_run(
        ...
        **synthesis_kwargs,
        ...
        if prob > 0:
            cutoff = (
                torch.empty([], dtype=torch.int64).random_(1, ws.shape[1]).type_as(z)
            )
            cutoff = torch.where(
                torch.rand([], device=ws.device) < prob,
                cutoff,
                torch.full_like(cutoff, ws.shape[1]),
            )
            ws[:, int(cutoff):] = self.mapping(
